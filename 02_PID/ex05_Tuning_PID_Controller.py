...
class PID_Controller(object):
    def __init__(self, reference, measure, step_time, P_Gain=0.8, D_Gain=1.2, I_Gain=0.05):
        self.Kp = P_Gain
        self.Kd = D_Gain
        self.Ki = I_Gain
...
if __name__ == "__main__":
    ...
    controller = PID_Controller(target_y, plant.y_measure[0][0], step_time,
                                P_Gain=1.5, D_Gain=2.7, I_Gain=0.3)  # 튜닝된 값
    ...
    plt.legend(loc="best")
    plt.grid(True)
    plt.title("Tuned PID Controller Result")
    plt.show()
