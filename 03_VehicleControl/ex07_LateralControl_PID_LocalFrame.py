...
class PD_Controller_Kinematic(object):
    def __init__(self, step_time, y_ref, y_current, P_Gain=0.1, D_Gain=0.2):
...
if __name__ == "__main__":
    ...
    controller = PD_Controller_Kinematic(step_time, 0.0, 0.0)
    ...
    for i in range(int(simulation_time / step_time)):
        ...
        if i == 70:
            print(f"[Step {i}] Points_ref = \n{Points_ref}")
            print(f"[Step {i}] frameconverter.LocalPoints = \n{frameconverter.LocalPoints}")
        ...
        # PD 제어
        controller.ControllerInput(lookahead_y, 0)
        delta_total = controller.u
    ...
    plt.legend()
    plt.grid(True)
    plt.title("PD Only Control (No Feedforward)")
    plt.axis("equal")
    plt.show()
